...
class uGLAD_GL(object):
    ...
    def fit(
        ...
        if mode == "missing":
            ...
            print(
                f"ERROR Please enter K-fold value in valid range [0, ), currently entered {k_fold}; Check mode {mode}"
            )
            sys.exit(0)
...
def init_uGLAD(lr, theta_init_offset=1.0, nF=3, H=3):
    ...
    """
    model = glad_params(theta_init_offset=theta_init_offset, nF=nF, H=H)
...
def run_uGLAD_direct(
    ...
    if trueTheta is not None:
        trueTheta = prepare_data.convertToTorch(trueTheta, req_grad=False)
    B, M, D = Xb.shape
    # NOTE: We fix the batch size B=1 for `uGLAD`
        ...
        for e in range(EPOCHS):
            ...
            if _loss < best_test_loss:  # and e%10==9:
                if VERBOSE and not e % PRINT_EVERY:
                    print(
                        f"Fold {_k}: epoch:{e}/{EPOCHS}: Updating the best model with test-loss {_loss}"
                    )
