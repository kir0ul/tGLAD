...
def function_plots_for_target(plot_dict):
    ...
    for i, source in enumerate(plot_dict[target].keys()):
        ax = plt.subplot(
            p + 1, int(num_sources / p), i + 1
        )  # (grid_x, grid_y, plot_num)
...
def plot_function(x, fx, title="plot of (x, fx)"):
...
def process_data_for_CI_graph(table, NORM="min_max", msg="", drop_duplicate=True):
    ...
    print(
        f"{msg}: Single value columns dropped: total {len(single_value_columns)}, columns {single_value_columns}"
    )
    ...
    if drop_duplicate:
        ...
        print(
            f"{msg}: Duplicates dropped: total {len(duplicate_columns)}, columns {duplicate_columns}"
        )
    ...
    print(
        f"{msg}: The processed table has sample {table.shape[0]} and features {table.shape[1]}"
    )
...
def plot_graph_compare(G, pos=None, title="", scale_wt=1, intensity=1):
    ...
    edge_width = [
        intensity * abs(float(G.edges[e]["weight"])) / scale_wt for e in G.edges
    ]
    plt.title(title, fontsize=20)
    n_edges = len(G.edges)
    if pos is None:
...
def compare_graphs(
    ...
    t1="Title1",
    t2="Title2",
    graph_type={"G1": "undirected", "G2": "undirected"},
):
    """
    ...
    print(
        f"DA: Common nodes: {len(common_nodes)}, Nodes in G1: {len(G1.nodes())}, Nodes in G2: {len(G2.nodes())}"
    )
    ...
    # unfreeze the graphs
    G1_int = (
        nx.Graph(G1_int) if graph_type["G1"] == "undirected" else nx.DiGraph(G1_int)
    )
    G2_int = (
        nx.Graph(G2_int) if graph_type["G2"] == "undirected" else nx.DiGraph(G2_int)
    )
    ...
    if graph_type["G1"] == "undirected":
        G1_int = nx.Graph(
            ((u, v, e) for u, v, e in G1_int.edges(data=True) if G_int.has_edge(u, v))
        )
        G1_unique = nx.Graph(
            ((u, v, e) for u, v, e in G1.edges(data=True) if not G_int.has_edge(u, v))
        )
        ...
        G1_unique = nx.DiGraph(
            ((u, v, e) for u, v, e in G1.edges(data=True) if not G_int.has_edge(u, v))
        )

    if graph_type["G2"] == "undirected":
        G2_int = nx.Graph(
            ((u, v, e) for u, v, e in G2_int.edges(data=True) if G_int.has_edge(u, v))
        )
        G2_unique = nx.Graph(
            ((u, v, e) for u, v, e in G2.edges(data=True) if not G_int.has_edge(u, v))
        )
    else:
        G2_int = nx.DiGraph(
            ((u, v, e) for u, v, e in G2_int.edges(data=True) if G_int.has_edge(u, v))
        )
        G2_unique = nx.DiGraph(
            ((u, v, e) for u, v, e in G2.edges(data=True) if not G_int.has_edge(u, v))
        )
    ...
    plot_graph_compare(
        G1_int,
        pos,
        title=t1 + ": Edges present in both graphs",
        scale_wt=scale_wt_G1,
        intensity=3,
    )
    plt.subplot(222)  # , figsize=(fig_size, fig_size))
    plot_graph_compare(
        G2_int, pos, title=t2 + ": Edges present in both graphs", scale_wt=scale_wt_G2
    )
    plt.subplot(223)  # , figsize=(fig_size, fig_size))
    plot_graph_compare(
        G1_unique, title=t1 + ": Unique edges", scale_wt=scale_wt_G1, intensity=3
    )
    plt.subplot(224)  # , figsize=(fig_size, fig_size))
    # G2_unique.remove_nodes_from(['no_mmorb', 'attend'])
    plot_graph_compare(
        G2_unique, title=t2 + ": Unique edges", scale_wt=scale_wt_G2
    )  # , get_image_bytes=True)
