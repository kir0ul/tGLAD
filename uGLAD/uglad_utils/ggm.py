...
def get_data(
    ...
    for b in range(batch_size):
        ...
        edge_connections = generateGraph(num_nodes, sparsity, typeG=typeG)
...
def viz_graph_from_precision(theta, column_names, sparsity=0.1, title=""):
    ...
    Gr, _, _ = graph_from_partial_correlations(rho, column_names, sparsity=sparsity)
...
def get_distribution_function(target, source, model_GGM, Xi, count=100):
    ...
    for _x in x_vals:  # expected_value calculation
        ...
        Xi_batch = pd.DataFrame(
            np.repeat(Xi.values, count, axis=0), columns=column_names
        )
...
def analyse_feature(target_feature, model_GGM, G, Xi=[]):
    ...
    for nbr in target_nbrs.keys():
        x, fx = get_distribution_function(target_feature, nbr, model_GGM, Xi)
