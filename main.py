...
def batch_time_series(df, window_size, batch_size, stride_length):
    ...
    windows = [
        df.iloc[i : i + window_size, :].values
        for i in range(0, num_windows * window_size - window_size, stride_length)
    ]

    # # Stack the windows into batches
    ts_batches = [
        windows[i : i + batch_size] for i in range(0, len(windows), batch_size)
    ]
    return ts_batches
...
def distance_fun(g_correlation_map_0, g_correlation_map_1):
    ...
    for edge1, edge2 in all_edges:
        weight_0 = (
            g_correlation_map_0.get_edge_data(edge1, edge2)["weight"]
            if g_correlation_map_0.get_edge_data(edge1, edge2)
            else 0
        )
        weight_1 = (
            g_correlation_map_1.get_edge_data(edge1, edge2)["weight"]
            if g_correlation_map_1.get_edge_data(edge1, edge2)
            else 0
        )
...
def get_label(extended_win, thresh_ddg_corr):
    ...
    for i in range(len(thresh_ddg_corr)):
        if thresh_ddg_corr[i] > 0:
            labels[
                max(0, i - extended_win) : min(len(labels) - 1, i + extended_win)
            ] = np.zeros(
                len(
                    labels[
                        max(0, i - extended_win) : min(
                            len(labels) - 1, i + extended_win
                        )
                    ]
                )
            )
    ...
    for i in tqdm(range(len(ts_batches))):
        ...
        for j in range(uglad_i.precision_.shape[0]):
            g_correlation = get_partial_correlation_from_precision(
                uglad_i.precision_[j], column_list
            )
